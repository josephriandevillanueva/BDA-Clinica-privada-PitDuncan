...
class pacientes:
    ...
    def consultar_datos_medicos(self,nombre,ap,am):
        try:
            ...
            cursor = collecciones['Pacientes'].aggregate([{"$match": {"nombre": nombre,"ap_paterno": ap,
                                    "ap_materno": am}}, {"$project": {"_id":0,"nombre":1,"apellido paterno":"$ap_paterno", 
    ...
    def mostrar_un_paciente(self,nombre,app,apm):
        try:
            cliente, collecciones = conectar(**connmongop)
            cursor = collecciones['Pacientes'].aggregate([{"$match":{"nombre":nombre,"ap_paterno":app,"ap_materno":apm}},
