...
def menu_inv(verficacion):
    ...
    with pes3:
        if verficacion:
            ...
            nombre = st.text_input("Ingrese el nombre del medicamento.")
            sustancia = st.text_input("Ingrese el nombre de la sustancia activa.")
            descripcion = st.text_input("Descripción")
            presentacion = st.text_input("Presentacion (ej: Frasco con 60 tabletas.)")
            marca = st.text_input("Marca")
            cod_b = st.text_input("Codigo de barras")
            recet = ["Si","No"]
            diccionario = {
                "Si":True,
                "No":False
            ...
            stock_t = st.text_input("Stock total")
            unidad = st.text_input("Unidad de medida (ej:caja,frasco,etc.)")
            precio = st.text_input("Precio por unidad")

            st.write("Datos del Lote")
            cod_lot = st.text_input("Codigo de lote")
            fecha_cad = st.date_input("Fecha de caducidad")
            cnt_dis = st.text_input("Cantidad disponible")

            st.write("Datos del proveedor")
            prov = st.text_input("Nombre del Proveedor")
            em_prov = st.text_input("Email proveedor")

            if st.button("Insertar medicamento."):
                i = inventario()
...
def comprar_medicina():
    ...
    with pest1:
        ...
        with st.form("formulario_comprar_med"):
            ...
            medicamento = st.text_input("Ingrese el nombre del medicamento o inicial para buscar.")
            ...
            nom_medicamento = st.text_input("Nombre del medicamento a comprar (ej. Amoxil 500mg).")
            cantidad = st.text_input("Ingrese la cantidad a comprar.")
        ...
        if len(st.session_state.carrito)>0:
            ...
            with st.form("Pagos"):
                ...
                with pesta1:
                    ...
                    num_tarjeta = st.text_input("Numero de tarjeta")
                    nom_prop = st.text_input("Nombre del propietario")
                    ...
                    with column2:
                        cvv = st.text_input("Ingrese CVV")

                    btn_linea = st.form_submit_button("Pagar")

                    if btn_linea:
                        if num_tarjeta and nom_prop and cvv:
                            i = inventario()
                            ...
                            st.warning("Favor de llenar los campos correspondientes para el pago en linea.")
